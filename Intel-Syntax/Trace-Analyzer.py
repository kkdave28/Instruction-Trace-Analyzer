                ...
                for o in range(0,len(operands)):
                    ...
                    for keys in register_lookup_table.keys():
                        if op.find(keys) != -1:
                            ...
                            operands[o] = operands[o].replace(keys, register_lookup_table[keys])
                            break
                        if op.startswith("0x"):
                    ...
                    if(len(corrupted_memory_addrs) != 0):
                        if(x[1][0].find('[') != -1):
                            for y in corrupted_memory_addrs:
                                if y in x[2]:
                                    if x[1][1].find("0x") != -1:
                                        print("Most recent corrupting instruction found: "+str(index)+ ": "+x[0]+" "+" ,".join(x[1]))
                                        output.write("\nMost recent corrupting instruction found: "+str(index)+ ": "+x[0]+" "+" ,".join(x[1]))
                                        return
                                    else:
                                        corrupted_op = x[1][1]
                                        corrupted_memory_addrs = []
